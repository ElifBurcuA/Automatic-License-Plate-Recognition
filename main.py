...
def get_approval(text, c):
    ...
    try:
        ...
        c.execute(cmm)
        names, surname, _, _ = c.fetchall()[0]
        full_name = names + " " + surname
...
def profile_ocr():
    if request.method == 'POST':
        ...
        return render_template('profile_ocr.html', upload=True, upload_image=filename, text=text,name=full_name,approval=approval)
...
@login_required
def database():
    status = "Add or remove vehicle information"
