...
def OCR(path, filename):
    ...
    roi_bgr1=cv2.resize(roi_bgr, None, fx = 2, fy = 2, interpolation = cv2.INTER_CUBIC)
    gray = cv2.cvtColor(roi_bgr1, cv2.COLOR_BGR2GRAY)
    gray=cv2.GaussianBlur(gray, (5, 5), 0)
    magic_color = apply_brightness_contrast(gray, brightness=40, contrast=70)
    cv2.imwrite('./static/roi/{}'.format(filename), roi_bgr)

    text = pt.image_to_string(magic_color, lang='eng', config='--oem 3 -l eng --psm 6')
    try:
